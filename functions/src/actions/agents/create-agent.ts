 ...
 * assignee selectors, which already read from `members` and need no changes
 * to show it. `agents/{agentId}` holds the operational bits (repo defaults,
 * concurrency limits) that don't belong on a `Member`.
 */
